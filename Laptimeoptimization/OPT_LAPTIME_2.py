...
def solve_min_time(csv_file='Nuerburgring.csv', ds=2):
    ...
    # ---- 优化参数 ----
    lambda_ddelta = 10 /3 # 10
    lambda_dF = 0.01 /3# 0.01
    lambda_dn = 1.0 /3# 10
    lambda_dxi = 5 /3 # 5
    lambda_dv = 0.1/3 # 1
    lambda_jerk = 1.0 /9 # 1
    ...
    for k in range(N):
        ...
        # ====================================
        if k > 0:
            ddelta = (Uk[0] - delta_prev) ** 2
            dF = (Uk[1] - F_prev) ** 2
            dn = (Xk[3] - Xk_prev[3]) ** 2
            dxi = (Xk[4] - Xk_prev[4]) ** 2
            dv = (Xk[0] - Xk_prev[0]) ** 2

            J += lambda_ddelta * ddelta
            J += lambda_dF * dF
            J += lambda_dn * dn
            J += lambda_dxi * dxi
            J += lambda_dv * dv
        ...
        if k > 1:
            jerk = (Uk[0] - 2 * delta_prev + delta_prev_prev) ** 2
            J += lambda_jerk * jerk
            J_jerk += jerk
    ...
    J_ddelta_func = ca.Function("J_ddelta", [ca.vertcat(*w)], [J_ddelta])
    J_dF_func = ca.Function("J_dF", [ca.vertcat(*w)], [J_dF])
    J_dn_func = ca.Function("J_dn", [ca.vertcat(*w)], [J_dn])
    J_dxi_func = ca.Function("J_dxi", [ca.vertcat(*w)], [J_dxi])
    J_dv_func = ca.Function("J_dv", [ca.vertcat(*w)], [J_dv])
    J_jerk_func = ca.Function("J_jerk", [ca.vertcat(*w)], [J_jerk])
    print("\n===== Cost Breakdown (before weights) =====")
    print(f"(Uk[0] - delta_prev)^2    : {float(J_ddelta_func(sol['x'])):.4f}")
    print(f"(Uk[1] - F_prev)^2        : {float(J_dF_func(sol['x'])):.4f}")
    print(f"(Xk[3] - Xk_prev[3])^2 (n): {float(J_dn_func(sol['x'])):.4f}")
    print(f"(Xk[4] - Xk_prev[4])^2(xi): {float(J_dxi_func(sol['x'])):.4f}")
    print(f"(Xk[0] - Xk_prev[0])^2 (v): {float(J_dv_func(sol['x'])):.4f}")
    print(f"jerk = d²delta/ds²        : {float(J_jerk_func(sol['x'])):.4f}")
