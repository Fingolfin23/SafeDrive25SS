...
% —— 反插值，得到 s_uniform 对应的原始 s_raw 坐标
...
%% 【1】确定有解的段
valid_segments = find( cellfun(@(s) strcmp(s,'Solve_Succeeded') || strcmp(s,'Converged'), exitStatus) );
...
% % 如果是第一段：v/beta/r 设默认值
% if isempty(X_start_val)
%     v_start = 5;
%     beta_start = 0;
%     r_start = 0;
% else
%     % 其它段：直接接续上段末端状态
%     v_start = X_start_val(5);
%     beta_start = X_start_val(4);
%     r_start = X_start_val(6);
% end
    ...
    opts = struct('print_time',0, 'ipopt',struct('tol',1e-1, 'max_iter',5e5,'acceptable_obj_change_tol', 1e1,'acceptable_tol', 1e-1));
