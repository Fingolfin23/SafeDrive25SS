...
xk = MX.sym('xk',7);
uk = MX.sym('uk',2);
% T_sym = MX.sym('T');          %  T_sym 
% dt_var = T_sym / N;
% xnext = xk + dt_var * f_dyn(xk, uk);
% euler_step = Function('euler_step', {xk, uk, T_sym}, {xnext});
...
dt_var=T/N
xnext = xk + dt_var * f_dyn(xk, uk);
euler_step = Function('euler_step', {xk, uk, T}, {xnext});
...
for k = 1:N
    xk = Xvar(:,k);
    uk = Uvar(:,k);
    xk1 = euler_step(xk, uk,T);   %  T
    opti.subject_to(Xvar(:,k+1) == xk1);
end

%%10. status（assure s=1，position constraint hard）
...
%% 12. solve
opti.solver('ipopt',struct('print_time',0,'ipopt',struct('tol',1e-1,'max_iter',5e5)));
sol = opti.solve();

% %% 13. visualization
Xopt = sol.value(Xvar);    % 7×(N+1)，1st row X， 2  Y， 5  v
% ref
...
subplot(2,1,1);
plot(XbP, YbP, 'k--','LineWidth',1.5); hold on;              % 
plot(Xopt(1,:), Xopt(2,:), 'b-','LineWidth',2);             % 
plot(X_corr_left,  Y_corr_left,  'k--','LineWidth',1);      % 
plot(X_corr_right, Y_corr_right, 'k--','LineWidth',1);      % 
axis equal; grid on;
